...
支持多指触控，摇杆响应的同时还能拖动屏幕转向
技能键固定位置，但是点击后要有技能预览的指示器
```
## 手机支持
```sh
...
### 基础属性
```sh
# 基础属性封装一个单独类，用来常驻数据，切换房间时保留数据
属性(BaseAttribute)：位置、血量、攻击力、防御力、模型、移动速度、攻击速度、射程
附带方法：血量加减、数据初始化
```
### 基础对象
```sh
# 对象性能开销更大，因此只保留当前房间的对象，对象有行为有方法
属性：绑定的数据引用、配合动作表现的特效、音效、动作等
方法：攻击、死亡等
```
### 怪物
```sh
